...
def check_pitch_jitter(audio_bytes):
    ...
    try:
        ...
        if len(pitch_contour) < 10:
            return {"pitch": 0, "jitter": 0, "shimmer": 0, "error": "Insufficient pitch data"}
        ...
        jitter = np.mean(period_diffs) / np.mean(pitch_periods) if np.mean(pitch_periods) > 0 else 0
        ...
        if len(frame_energies) > 1:
            ...
            shimmer = np.mean(energy_diffs) / np.mean(frame_energies) if np.mean(frame_energies) > 0 else 0
        else:
            shimmer = 0
    ...
    try:
        ...
        if reference_bytes is not None:
            ...
            if segment_length > sr1:  # At least 1 second per segment
                segments = [y1[i:i+segment_length] for i in range(0, len(y1), segment_length)]
                ...
                if len(segment_features) >= 2:
                    ...
                    for i in range(len(segment_features)):
                        for j in range(i+1, len(segment_features)):
                            sim = calculate_feature_similarity(segment_features[i], segment_features[j])
        ...
        return {
            ...
            "analysis_type": "reference_match" if reference_bytes else "consistency_check"
        }
...
def calculate_feature_similarity(features1, features2):
    ...
    try:
        ...
        if 'chroma_mean' in features1 and 'chroma_mean' in features2:
            chroma_sim = np.corrcoef(features1['chroma_mean'], features2['chroma_mean'])[0, 1]
