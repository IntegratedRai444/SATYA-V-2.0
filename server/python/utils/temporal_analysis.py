    ...
    try:
        ...
        results = {
            ...
            "inconsistencies": _detect_temporal_inconsistencies(
                flow_metrics, diff_metrics, motion_analysis, threshold
            ),
        }
...
def _calculate_optical_flow_metrics(frames: List[np.ndarray]) -> List[Dict]:
    ...
    flow_metrics.append({"mean_flow": 0.0, "std_flow": 0.0, "max_flow": 0.0})

    for i in range(1, len(frames)):
        # Calculate dense optical flow using Farneback method
        flow = cv2.calcOpticalFlowFarneback(
            prev_gray, frames[i], None, 0.5, 3, 15, 3, 5, 1.2, 0
        )
...
def _calculate_frame_differences(frames: List[np.ndarray]) -> List[Dict]:
    ...
    diff_metrics.append({"mse": 0.0, "ssim": 1.0, "diff_ratio": 0.0})
    ...
    for i in range(1, len(frames)):
        ...
        diff_metrics.append(
            {"mse": float(mse), "ssim": float(ssim), "diff_ratio": float(diff_ratio)}
        )
...
def _analyze_motion_patterns(
    flow_metrics: List[Dict], window_size: int, threshold: float
) -> Dict[str, Any]:
    ...
    moving_avg = np.convolve(
        mean_flows, np.ones(window_size) / window_size, mode="valid"
    )
...
def _detect_temporal_inconsistencies(
    ...
    for i in range(1, len(flow_metrics)):
        ...
        flow_change = abs(
            flow_metrics[i]["mean_flow"] - flow_metrics[i - 1]["mean_flow"]
        )
        ...
        if (
            flow_change > threshold
            or diff_change > threshold * 100
            or ssim_drop > threshold
        ):
