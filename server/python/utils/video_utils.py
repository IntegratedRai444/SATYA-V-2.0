                    ...
                    results.append(
                        {
                            ...
                            "authenticity_score": frame_result.get(
...
def analyze_video_advanced(
    video_path: str, output_dir: str, generate_overlay: bool = True
) -> Dict:
    """
    ...
    """
    from .audio_analysis import check_av_sync, extract_audio_features
    ...
    try:
        ...
        result = {
            ...
            "audio_analysis": {"features": audio_features, "av_sync": av_sync},
            "timeline": timeline,
            "overlay_video": overlay_path,
            "summary": {
                "is_manipulated": any(
                    fr.get("is_manipulated", False) for fr in frame_results
                ),
                "manipulation_score": sum(
                    fr.get("manipulation_score", 0) for fr in frame_results
                )
                / len(frame_results)
                if frame_results
                else 0,
                "confidence": sum(fr.get("confidence", 0) for fr in frame_results)
                / len(frame_results)
                if frame_results
                else 0,
                "anomalies_detected": sum(
                    1 for fr in frame_results if fr.get("has_anomalies", False)
                ),
...
def generate_timeline(frame_results: List[Dict]) -> Dict:
    ...
    manipulated_frames = sum(
        1 for fr in frame_results if fr.get("label") == "MANIPULATED"
    )
    ...
    for i in range(1, len(smoothed_confidences)):
        if smoothed_confidences[i] < smoothed_confidences[i - 1] * (1 - threshold):
            drop_amount = (
                smoothed_confidences[i - 1] - smoothed_confidences[i]
            ) / smoothed_confidences[i - 1]
    ...
    return {
        "anomalies": [
            i for i, fr in enumerate(frame_results) if fr.get("label") == "MANIPULATED"
        ],
        "confidence_trend": [
...
def generate_video_overlay(
    video_bytes: bytes, frame_results: List[Dict], output_path: str = None
) -> str:
    """
    ...
    try:
        ...
        while success and frame_idx < total_frames and frame_idx < len(frame_results):
            ...
            cv2.putText(
                frame, label, (10, 60), cv2.FONT_HERSHEY_SIMPLEX, 1, label_color, 2
            )
