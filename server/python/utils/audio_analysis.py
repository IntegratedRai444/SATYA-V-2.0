...
def extract_audio_features(video_path: str) -> Dict[str, Any]:
    ...
    try:
        ...
        features = {
            ...
            "mfcc": [float(x) for x in np.mean(librosa.feature.mfcc(y=y, sr=sr, n_mfcc=13), axis=1)],
            "chroma_stft": [float(x) for x in np.mean(librosa.feature.chroma_stft(y=y, sr=sr), axis=1)],
        }
...
def detect_audio_artifacts(audio_features: Dict[str, Any]) -> List[Dict]:
    ...
    if "zero_crossing_rate" in audio_features and "rms_energy" in audio_features:
        ...
        if audio_features["zero_crossing_rate"] > 0.1 and audio_features["rms_energy"] < 0.01:
            artifacts.append({
    ...
    if "spectral_centroid" in audio_features and "spectral_bandwidth" in audio_features:
        if audio_features["spectral_centroid"] < 500 and audio_features["spectral_bandwidth"] > 3000:
            artifacts.append({
