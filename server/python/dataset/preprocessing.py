...
def create_dataloaders(
    ...
    train_dataset = DeepfakeDataset(
        data_dir=data_dir, split="train", transform=get_transforms("train")
    )

    val_dataset = DeepfakeDataset(
        data_dir=data_dir, split="val", transform=get_transforms("val")
    )

    test_dataset = DeepfakeDataset(
        data_dir=data_dir, split="test", transform=get_transforms("test")
    )
...
class FaceExtractor:
    ...
    def __init__(self, face_size: int = 224):
        ...
        try:
            ...
            self.detector = MTCNN(
                image_size=face_size, margin=20, keep_all=False, post_process=True
            )
            self.use_mtcnn = True
        except ImportError:
