...
CLIENT_DIR = os.path.join(os.path.dirname(os.path.dirname(os.path.dirname(os.path.dirname(os.path.abspath(__file__))))), "client", "src")
...
def test_auth_flow():
    ...
    try:
        reg_res = session.post(f"{API_URL}/api/auth/register", json={
            "username": username,
            "email": email,
            "password": password,
            "full_name": "Test User"
    ...
    print("   Logging in...")
    try:
        login_res = session.post(f"{API_URL}/api/auth/login", data={
            "username": username,
            "password": password
        })
    ...
    try:
        ...
        if user_data["username"] != username:
            print(f"   ❌ User mismatch: expected {username}, got {user_data['username']}")
            return False
    except Exception as e:
    ...
    try:
        ...
        refresh_res = session.post(f"{API_URL}/api/auth/refresh", json={"refresh_token": refresh_token})
        ...
        print(f"   ⚠️ Refresh token test skipped/failed (might need specific payload format): {e}")
