...
from database.base import engine, SessionLocal, Base, get_db
...
from database.team import TeamMember, TeamInvitation
...
class DatabaseManager:
    ...
    def get_analysis_stats(self) -> dict:
        ...
        try:
            ...
            # Count by type
            image_count = db.query(AnalysisResult).filter(AnalysisResult.file_type == 'image').count()
            video_count = db.query(AnalysisResult).filter(AnalysisResult.file_type == 'video').count()
            audio_count = db.query(AnalysisResult).filter(AnalysisResult.file_type == 'audio').count()
            text_count = db.query(AnalysisResult).filter(AnalysisResult.file_type == 'text').count()
            
            # Count by label
            authentic_count = db.query(AnalysisResult).filter(AnalysisResult.label == 'authentic').count()
            deepfake_count = db.query(AnalysisResult).filter(AnalysisResult.label == 'deepfake').count()
            suspicious_count = db.query(AnalysisResult).filter(AnalysisResult.label == 'suspicious').count()
            
            # Average processing time
            avg_time = db.query(func.avg(AnalysisResult.processing_time)).scalar() or 0.0
    ...
    def get_daily_analysis_stats(self, days: int = 7) -> dict:
        ...
        try:
            ...
            daily_stats = db.query(
                ...
                func.sum(case((AnalysisResult.label == 'deepfake', 1), else_=0)).label('deepfakes')
            ).filter(
    ...
    def get_active_users_count(self, days: int = 30) -> int:
        ...
        try:
            ...
            count = db.query(func.count(distinct(AnalysisResult.user_id)))\
    ...
    def save_user_feedback(self, analysis_id: int, feedback_type: str, 
                          actual_label: str = None, comment: str = None, 
    ...
    def get_false_positive_rate(self) -> float:
        ...
        try:
            ...
            feedbacks = db.query(UserFeedback, AnalysisResult)\
            ...
            if len(feedbacks) < 10:
                ...
                logger.warning("⚠️ Insufficient feedback data for FPR calculation, using default 0.05")
                return 0.05
            ...
            for feedback, analysis in feedbacks:
                model_predicted_fake = analysis.label.lower() in ['deepfake', 'suspicious']
                user_says_authentic = feedback.actual_label and feedback.actual_label.lower() == 'authentic'
                user_says_fake = feedback.actual_label and feedback.actual_label.lower() in ['deepfake', 'fake']
            ...
            logger.info(f"📊 Calculated FPR: {fpr:.4f} (FP={false_positives}, TN={true_negatives})")
            return float(fpr)
    ...
    def get_team_members(self) -> List[Dict]:
        ...
        db = self.get_session()
        try:
            members = db.query(TeamMember, User)\
            ...
            for team_member, user in members:
                result.append({
                    ...
                    'joinDate': team_member.joined_at.isoformat() if team_member.joined_at else None,
    ...
    def invite_team_member(self, email: str, role: str, invited_by_user_id: int) -> bool:
        """
