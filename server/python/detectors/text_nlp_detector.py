...
class TextNLPDetector:
            ...
            if self.config["models"]["sentiment"]["enabled"]:
                try:
                    self.sentiment_analyzer = pipeline(
                        "sentiment-analysis", device=0 if self.device == "cuda" else -1
                    )
    ...
    def detect(self, text: str, metadata: Optional[Dict] = None) -> Dict:
        ...
        try:
            ...
            # Combine all scores
            final_score, confidence, label = self._combine_all_nlp_scores(
                results["details"]
            )
            ...
            logger.info(
                f"✅ Text analysis complete: {label} ({final_score:.3f}, confidence: {confidence:.3f})"
            )
    ...
    def _generate_nlp_explanation(self, results: Dict) -> str:
        ...
        if label == "human_written":
            explanations.append(
                f"Text appears to be human-written (score: {score:.2f}, confidence: {confidence:.2f})."
            )
        elif label == "ai_generated":
            explanations.append(
                f"Text shows strong signs of AI generation (score: {score:.2f}, confidence: {confidence:.2f})."
            )
        else:
            explanations.append(
                f"Text shows suspicious characteristics (score: {score:.2f}, confidence: {confidence:.2f})."
            )
