...
class AudioDetector:
    ...
    def _default_config(self) -> Dict:
        ...
        return {
            ...
            "thresholds": {"authentic": 0.7, "suspicious": 0.4, "deepfake": 0.4},
            "audio_processing": {
    ...
    def _load_pretrained_model(self):
        ...
        try:
            ...
            logger.error(
                "❌ Could not load any Wav2Vec2 model. ML detection will be disabled."
            )
            return False
    ...
    def detect(self, audio_path: str, timeout: int = 30) -> Dict:
        ...
        try:
            ...
            # Combine all scores
            final_score, confidence, label = self._combine_all_audio_scores(
                results["details"]
            )
            ...
            logger.info(
                f"✅ Audio analysis complete: {label} ({final_score:.3f}, confidence: {confidence:.3f})"
            )
        ...
        try:
            if LIBROSA_AVAILABLE:
                ...
                large_changes = np.sum(
                    np.abs(spec_diff) > np.percentile(np.abs(spec_diff), 95)
                )
    ...
    def _analyze_frequency_domain(self, audio_array: np.ndarray, sr: int) -> Dict:
        ...
        try:
            ...
            # 1. Spectral flatness
            spectral_flatness = np.exp(np.mean(np.log(positive_fft + 1e-10))) / (
                np.mean(positive_fft) + 1e-10
            )

            # 2. Frequency distribution
            low_freq_energy = np.sum(positive_fft[positive_freqs < 1000])
            mid_freq_energy = np.sum(
                positive_fft[(positive_freqs >= 1000) & (positive_freqs < 4000)]
            )
    ...
    def _generate_audio_explanation(self, results: Dict) -> str:
        ...
        if label == "authentic":
            explanations.append(
                f"Audio appears to be authentic (score: {score:.2f}, confidence: {confidence:.2f})."
            )
        elif label == "deepfake":
            explanations.append(
                f"Audio shows strong signs of voice cloning/synthesis (score: {score:.2f}, confidence: {confidence:.2f})."
            )
        else:
            explanations.append(
                f"Audio shows suspicious characteristics (score: {score:.2f}, confidence: {confidence:.2f})."
            )

        # Artifacts
        if results.get("artifacts_detected"):
            explanations.append(
                f"Detected artifacts: {', '.join(results['artifacts_detected'][:3])}."
            )
        ...
        if "ml_classification" in results.get("details", {}):
            ...
            if "fake_probability" in ml and ml["fake_probability"] > 0.6:
                explanations.append(
                    f"ML model detected {ml['fake_probability']*100:.1f}% probability of synthesis."
                )
