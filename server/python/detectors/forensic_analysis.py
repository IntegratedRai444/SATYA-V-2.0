...
def create_mesonet(pretrained=False):
    ...
    if pretrained:
        try:
            ...
            model_path = (
                Path(__file__).parent.parent / "models" / "mesonet_pretrained.pth"
            )
            ...
            if model_path.exists():
                ...
                logger.warning(
                    f"⚠️ Pretrained MesoNet weights not found at {model_path}, using random initialization"
                )
        except Exception as e:
            logger.warning(
                f"⚠️ Failed to load pretrained MesoNet weights: {e}, using random initialization"
            )
