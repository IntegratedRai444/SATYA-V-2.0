...
class MultimodalFusionDetector:
    ...
    def __init__(self, config: Optional[Dict] = None):
        ...
        self.text_nlp_detector = TextNLPDetector() if TEXT_NLP_DETECTOR_AVAILABLE else None
    ...
    def detect_multimodal(
        ...
        try:
            ...
            if len(modality_scores) < self.config['min_modalities']:
                results['warnings'] = [f"Only {len(modality_scores)} modalities available, need at least {self.config['min_modalities']}"]
            ...
            if self.config['enable_ensemble']:
                fusion_result = self._ensemble_fusion(
                    modality_scores,
                    results.get('cross_modal_analysis', {})
                )
            ...
            results['recommendations'] = self._generate_multimodal_recommendations(results)
            
            logger.info(f"✅ Multimodal analysis complete: {results['label']} ({results['authenticity_score']:.3f})")
    ...
    def _analyze_cross_modal_consistency(self, individual_results: Dict) -> Dict:
        ...
        try:
            ...
            if consistency_checks:
                avg_consistency = np.mean([c['consistency'] for c in consistency_checks])
    ...
    def _ensemble_fusion(self, modality_scores: Dict, cross_modal: Dict) -> Dict:
        ...
        try:
            ...
            base_score = sum(weighted_scores) / total_weight if total_weight > 0 else 0.5
            
            # 2. Apply cross-modal consistency bonus/penalty
            cross_modal_consistency = cross_modal.get('average_consistency', 1.0)
            cross_modal_adjustment = (cross_modal_consistency - 0.5) * self.cross_modal_weight
    ...
    def _generate_multimodal_explanation(self, results: Dict) -> str:
        ...
        # Main verdict
        explanations.append(f"Multimodal analysis ({len(modalities)} modalities) indicates: {label.upper()}")
        explanations.append(f"Overall score: {score:.2f}, Confidence: {confidence:.2f}.")
        ...
        for modality in modalities:
            if modality in results.get('individual_results', {}):
                ...
                explanations.append(f"{modality.capitalize()}: {mod_label} ({mod_score:.2f}).")
    ...
    def _generate_multimodal_recommendations(self, results: Dict) -> List[str]:
        ...
        if label == 'deepfake':
            recommendations.append("STRONG EVIDENCE of deepfake manipulation across multiple modalities")
            recommendations.append("Do not trust this content for verification")
        elif label == 'suspicious':
            recommendations.append("Multiple modalities show suspicious characteristics")
            recommendations.append("Recommend additional verification")
        else:
            recommendations.append("Content appears authentic across analyzed modalities")
        ...
        if 'cross_modal_analysis' in results:
            ...
            if cross_modal.get('suspicious_pairs', 0) > 0:
                recommendations.append(f"WARNING: {cross_modal['suspicious_pairs']} cross-modal inconsistencies detected")
        
        # Modality-specific recommendations
        if len(modalities) < 3:
            recommendations.append(f"Only {len(modalities)} modalities analyzed - consider analyzing more for higher confidence")
