...
def preprocess_video_frames(frames: List[np.ndarray], target_size: Tuple[int, int] = (224, 224)) -> torch.Tensor:
...
def predict_video_deepfake(frames: List[np.ndarray], model: Optional[nn.Module] = None,
    ...
    if model is None:
        ...
        try:
            from pathlib import Path
            model_path = Path(__file__).parent.parent / 'models' / 'video_deepfake_detector.pth'
            
            if model_path.exists():
                state_dict = torch.load(model_path, map_location=device)
                model.load_state_dict(state_dict)
                logger.info(f"✅ Loaded pretrained video model weights from {model_path}")
            else:
                logger.warning(f"⚠️ Pretrained video model weights not found at {model_path}, using random initialization")
        except Exception as e:
            logger.warning(f"⚠️ Failed to load pretrained video model weights: {e}, using random initialization")
