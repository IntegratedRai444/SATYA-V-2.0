...
class TemporalConvNet(nn.Module):
    ...
    def __init__(self, in_channels: int = 3, num_classes: int = 2):
        ...
        self.conv1 = nn.Sequential(
            nn.Conv3d(in_channels, 64, kernel_size=(3, 7, 7), stride=(1, 2, 2), padding=(1, 3, 3)),
            nn.BatchNorm3d(64),
            nn.ReLU(),
    ...
    def forward(self, x: torch.Tensor) -> Dict[str, torch.Tensor]:
        ...
        x = x.contiguous().view(batch_size, x.size(1), -1)  # (batch_size, frames/4, 256*14*14)
        
        # Apply temporal attention
        features, attention_weights = self.temporal_attention(x)  # (batch_size, 256*14*14)
...
class VideoDeepfakeDetector(nn.Module):
    ...
    def forward(self, x: torch.Tensor) -> Dict[str, torch.Tensor]:
        ...
        frame_features = torch.stack(frame_features, dim=2)  # (batch_size, features, num_frames, h', w')
...
def load_video_model(checkpoint_path: str, device: str = 'cuda' if torch.cuda.is_available() else 'cpu') -> VideoDeepfakeDetector:
