...
from torchvision.models.swin_transformer import SwinTransformer, Swin_S_Weights
...
class MultiModalDeepfakeDetector(nn.Module):
    ...
    def __init__(self, num_classes: int = 2, pretrained: bool = True, use_swin: bool = True):
        super().__init__()
        ...
        if use_swin:
            ...
            self.effnet_backbone = efficientnet_v2_s(weights=EfficientNet_V2_S_Weights.IMAGENET1K_V1 if pretrained else None)
        ...
        self.cross_scale_fusion = CrossScaleFeatureFusion(
            in_channels_list=[256] * 4,  # FPN outputs same channels
            out_channels=256
        )
...
class ModelManager:
    ...
    def load_model(self, weights_path: Optional[str] = None) -> None:
        ...
        self.logger.info(f"Loading {self.model_type.upper()} model on device: {self.device}")
    ...
    def predict(self, image: Union[np.ndarray, Image.Image], 
        ...
        with torch.no_grad():
            ...
            result = {
                ...
                'manipulation_type': torch.softmax(outputs.get('manipulation_type', torch.zeros(1, 5)), 1)[0].cpu().numpy()
            }
...
def preprocess_image(image: Union[np.ndarray, Image.Image], 
                   target_size: Tuple[int, int] = (384, 384)) -> torch.Tensor:
...
def predict_deepfake(
    ...
    try:
        ...
        manip_types = ['FaceSwap', 'Face2Face', 'NeuralTextures', 'DeepFake', 'FaceShifter']
        ...
        analysis['manipulation_types'] = {
            name: float(prob) 
            for name, prob in zip(manip_types, manip_probs)
        }
...
def main():
    ...
    parser.add_argument('--model', default='swin', choices=['swin', 'efficientnet'],
                       help='Model architecture to use')
    parser.add_argument('--device', default=None, help='Device to run on (cuda or cpu)')
    parser.add_argument('--attention', action='store_true', 
                       help='Generate attention visualization')
    ...
    try:
        ...
        print(f"{'='*50}")
        print(f"Prediction: {result['label'].upper()} (confidence: {result['confidence']*100:.1f}%)")
        print("\nExplanation:")
        ...
        if args.attention and 'analysis' in result and 'attention_map' in result['analysis']:
