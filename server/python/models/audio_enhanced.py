...
class AudioPreprocessor:
    ...
    def __init__(self, sample_rate: int = 16000, n_fft: int = 512, 
                 hop_length: int = 128, n_mels: int = 64):
    ...
    def _spectral_centroid(self, waveform: torch.Tensor) -> torch.Tensor:
        ...
        freqs = torch.linspace(0, self.sample_rate//2, spec.shape[-2], 
                              device=waveform.device)
        centroid = torch.sum(freqs.unsqueeze(-1) * spec, dim=-2) / (torch.sum(spec, dim=-2) + 1e-6)
        return centroid
    ...
    def _spectral_bandwidth(self, waveform: torch.Tensor) -> torch.Tensor:
        ...
        freqs = torch.linspace(0, self.sample_rate//2, spec.shape[-2], 
                              device=waveform.device).unsqueeze(-1)
        bandwidth = torch.sum((freqs - centroid).pow(2) * spec, dim=-2) / (
            torch.sum(spec, dim=-2) + 1e-6)
        return bandwidth
    
    def _spectral_contrast(self, waveform: torch.Tensor, n_bands: int = 6) -> torch.Tensor:
    ...
    def _voice_activity_detection(self, waveform: torch.Tensor, 
