...
class EnhancedDeepfakeDetector:
    ...
    def analyze_image(self, image_buffer: bytes) -> Dict[str, Any]:
        ...
        try:
            ...
            analysis_results['technical_details']['processing_time_seconds'] = processing_time
    ...
    def _analyze_with_real_models(self, image: Image.Image, image_buffer: bytes) -> Dict[str, Any]:
        ...
        try:
            ...
            model_paths = [
                ...
                (base_path / "efficientnet_b4_deepfake.bin", "EfficientNet-B4 Deepfake Detector"),
                (base_path / "xception_c23.pth", "Xception C23 (FaceForensics++)"),
                (base_path / "dfdc_efficientnet_b7/pytorch_model.bin", "EfficientNet-B7 (DFDC Winner)"),
            ]
            ...
            try:
                ...
                if any('layer1' in key for key in model_keys):
                    ...
                    try:
                        ...
                        logger.warning(f"Strict loading failed for {model_name}, trying flexible loading: {e}")
                        ...
                        logger.info(f"✅ Loaded {model_name} with flexible matching ({len(filtered_state)}/{len(state_dict)} layers)")
                    ...
                    try:
                        ...
                        logger.warning(f"EfficientNet loading failed: {e}, using ResNet substitute")
                    ...
                    logger.warning(f"Unknown architecture in {model_name}, creating compatible model")
                    model = models.resnet50(pretrained=True)
                    model.fc = torch.nn.Linear(model.fc.in_features, 2)
                ...
                logger.info("Creating ultimate fallback model with pre-trained ImageNet weights")
                model = models.resnet50(pretrained=True)
                model.fc = torch.nn.Linear(model.fc.in_features, 2)
                model.eval()
                model.to(device)
            ...
            if face_predictions:
                ...
                most_suspicious = max(face_predictions, key=lambda x: x['fake_probability'])
    ...
    def _create_working_ai_result(self, image: Image.Image, image_buffer: bytes) -> Dict[str, Any]:
        ...
        try:
            ...
            findings = [
                ...
                f"Real probability: {real_prob:.3f}",
                f"Fake probability: {fake_prob:.3f}",
                "AI model indicates " + ("authentic content" if authenticity == 'AUTHENTIC MEDIA' else "potential manipulation" if authenticity == 'MANIPULATED MEDIA' else "inconclusive results")
            ]
    ...
    def _analyze_with_heuristics(self, image: Image.Image, image_buffer: bytes) -> Dict[str, Any]:
        ...
        try:
            ...
            findings = self._generate_findings(
                metadata_score, stats_score, freq_score, 
                texture_score, color_score, authenticity
            )
    ...
    def _analyze_metadata(self, image_buffer: bytes) -> float:
        ...
        try:
            ...
            if exif_data:
                ...
                if any(tag in exif_data for tag in [272, 271, 306]):  # Make, Model, DateTime
                    score += 0.1
    ...
    def _analyze_image_statistics(self, img_array: np.ndarray) -> float:
        ...
        try:
            ...
            if 6.0 <= entropy <= 8.0:  # Natural images typically have entropy in this range
                score += 0.2
    ...
    def _generate_findings(self, metadata_score: float, stats_score: float, 
                          freq_score: float, texture_score: float, 
                          color_score: float, authenticity: str) -> List[str]:
    ...
    def _analyze_texture_patterns(self, image_array: np.ndarray) -> Dict[str, Any]:
        ...
        try:
            ...
            return {
                ...
                'overall_texture_score': round((texture_uniformity + edge_consistency + noise_score + gradient_score) / 4, 3)
            }
    ...
    def _analyze_frequency_domain(self, image_array: np.ndarray) -> Dict[str, Any]:
        ...
        try:
            ...
            return {
                ...
                'overall_frequency_score': round((dct_score + fft_score + wavelet_score) / 3, 3)
            }
    ...
    def _analyze_metadata(self, image: Image.Image) -> Dict[str, Any]:
        ...
        try:
            ...
            return {
                ...
                'overall_metadata_score': round(metadata_completeness * 0.7 + (1 - len(software_indicators['suspicious']) * 0.1), 3)
            }
    ...
    def _analyze_image_statistics(self, image_array: np.ndarray) -> Dict[str, Any]:
        ...
        try:
            ...
            return {
                ...
                'overall_statistical_score': round((histogram_score + contrast_brightness['score'] + saturation_score) / 3, 3)
            }
            
        except Exception as e:
            logger.warning(f"Statistical analysis failed: {e}")
            return {
                'color_distribution': {'red_mean': 128, 'green_mean': 128, 'blue_mean': 128},
                'histogram_score': 0.7,
                'contrast_brightness': {'contrast': 0.5, 'brightness': 0.5, 'score': 0.7},
    ...
    def _analyze_compression_artifacts(self, image_array: np.ndarray) -> Dict[str, Any]:
        ...
        try:
            ...
            return {
                ...
                'double_compression_confidence': round(double_compression['confidence'], 3),
                'overall_compression_score': round((block_artifacts + compression_consistency) / 2, 3)
            }
...
# Helper methods for detailed analysis
    def _calculate_facial_symmetry(self, face_region: np.ndarray) -> float:
        ...
        try:
            ...
            correlation = cv2.matchTemplate(left_half.astype(np.float32), right_half.astype(np.float32), cv2.TM_CCOEFF_NORMED)
    ...
    def _analyze_eye_regions(self, gray: np.ndarray, faces: List) -> Dict[str, Any]:
        ...
        try:
            ...
            eye_consistency = min(1.0, eyes_detected / (len(faces) * 2))  # Expect 2 eyes per face
            ...
            return {'natural_blinking': True, 'eye_consistency': 0.7, 'eyes_detected': 0}
    ...
    def _analyze_skin_texture(self, image_array: np.ndarray, faces: List) -> float:
        ...
        try:
            ...
            for (x, y, w, h) in faces:
                ...
                score = 1.0 - abs(skin_std - 30) / 100  # Normalize around expected skin texture
    ...
    def _check_software_signatures(self, exif_data: Dict) -> Dict[str, List]:
        ...
        try:
            ...
            legitimate_software = ['Camera', 'iPhone', 'Canon', 'Nikon', 'Sony', 'Adobe Lightroom']
            suspicious_software = ['Photoshop', 'GIMP', 'FaceSwap', 'DeepFace', 'AI', 'Generated']
            
            found_legitimate = [sw for sw in legitimate_software if sw.lower() in str(software_field).lower()]
            found_suspicious = [sw for sw in suspicious_software if sw.lower() in str(software_field).lower()]
    ...
    def _find_suspicious_metadata(self, exif_data: Dict) -> List[str]:
        ...
        try:
            ...
            if any(term in software for term in ['ai', 'generated', 'synthetic', 'deepfake']):
        ...
        try:
            ...
            for i in range(0, h - 16, 8):
                for j in range(0, w - 16, 8):
                    ...
                    edge1 = block1[:, -1]
                    edge2 = block2[:, 0]
                    discontinuity = np.mean(np.abs(edge1.astype(float) - edge2.astype(float)))
    ...
    def _detect_double_compression(self, image_array: np.ndarray) -> Dict[str, Any]:
        ...
        try:
            ...
            peaks = np.where(coeff_hist > np.mean(coeff_hist) + 2 * np.std(coeff_hist))[0]
    ...
    def _generate_key_findings(self, analysis_results: Dict[str, Any], overall_score: float) -> List[str]:
        ...
        try:
            ...
            if texture.get('noise_pattern_score', 0) < 0.5:
                findings.append("Unusual noise patterns detected - possible AI generation")
            
            # Metadata findings
            metadata = analysis_results.get('metadata_analysis', {})
            if metadata.get('suspicious_indicators'):
                findings.append(f"Suspicious metadata: {', '.join(metadata['suspicious_indicators'])}")
            ...
            if compression.get('double_compression_detected', False):
                findings.append("Double JPEG compression detected - indicates re-editing")
            ...
            if not findings:
                findings.append("Analysis completed - see detailed results for more information")
    ...
    def _generate_risk_assessment(self, score: float, analysis_results: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            if score >= 0.8:
                ...
                risk_level = "HIGH"
                risk_description = "Image shows mixed indicators - verification recommended"
            else:
                risk_level = "CRITICAL"
    ...
    def _generate_recommendations(self, score: float, analysis_results: Dict[str, Any]) -> List[str]:
        ...
        try:
            if score < 0.4:
                ...
                recommendations.append("Do not use for critical decisions without verification")
            elif score < 0.7:
                recommendations.append("Cross-reference with other sources")
                recommendations.append("Consider context and source reliability")
            else:
                recommendations.append("Image appears authentic based on technical analysis")
                recommendations.append("Continue standard verification procedures")
            
            # Specific recommendations based on findings
            metadata = analysis_results.get('metadata_analysis', {})
            if not metadata.get('exif_present', False):
                recommendations.append("Request original file with metadata if possible")
            
            facial = analysis_results.get('facial_analysis', {})
            if facial.get('faces_detected', 0) > 0 and facial.get('overall_facial_score', 1) < 0.6:
