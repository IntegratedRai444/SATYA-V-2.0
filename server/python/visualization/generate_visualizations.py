...
def generate_training_history():
    ...
    # Loss plot
    axes[0].plot(
        epochs, train_loss, label="Training Loss", linewidth=2, marker="o", markersize=3
    )
    axes[0].plot(
        epochs, val_loss, label="Validation Loss", linewidth=2, marker="s", markersize=3
    )
...
def generate_confusion_matrix():
    ...
    cm = np.array(
        [[169200, 10800], [9720, 170280]]  # Real: 94% correct  # Fake: 94.6% correct
    )
    ...
    plt.title(
        "Confusion Matrix - Deepfake Detection Model",
        fontsize=16,
        fontweight="bold",
        pad=20,
    )
...
def generate_roc_curve():
    ...
    plt.figure(figsize=(10, 8))
    plt.plot(
        fpr, tpr, color="darkorange", lw=3, label=f"ROC curve (AUC = {roc_auc:.2f})"
    )
    plt.plot(
...
def generate_model_comparison():
    ...
    axes[1].set_title(
        "Model Inference Speed Comparison", fontsize=14, fontweight="bold"
    )
...
def generate_learning_rate_schedule():
    ...
    plt.title(
        "Learning Rate Schedule (ReduceLROnPlateau)", fontsize=16, fontweight="bold"
    )
    plt.yscale("log")
    plt.grid(True, alpha=0.3)
    plt.tight_layout()
...
def generate_dataset_distribution():
    ...
    axes[0].set_title(
        "Training Dataset Distribution\n(1.44M samples)", fontsize=14, fontweight="bold"
    )
